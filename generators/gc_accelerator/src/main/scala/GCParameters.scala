...
class Ctrl2Top extends Bundle with HWParameters {
  ...
  val TaskQueue_Bottom = Input(UInt(32.W))
...
class GCTopIO extends Bundle{
  ...
  val io_mmu2llc_RequestSize_valid = Output(Bool())
  val io_mmu2llc_RequestSize_payload = Output(UInt(6.W))
  ...
  val io_ctrl2top_TaskQueue_Bottom = Input(UInt(32.W))
  ...
  val io_ctrl2top_CompressedFlag = Input(UInt(32.W))
