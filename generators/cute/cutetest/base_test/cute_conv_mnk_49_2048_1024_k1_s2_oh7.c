...
#include "conv_value_mnk_49_2048_1024_k1_s2_oh7.h"
...
//                                            VVVVV 每个ReduceDim需要Load的周期数 
//AML的总load请求数量 = (Tile_A load请求数量) * Tensor_K * Taskctrl拆分的任务次数 =  (49) * 2 * 512 = 50176 [A_LOAD]
//                                            ^^^^ = 2   ^^^^ = 49 * 2048 * 1024 --> 1 x 32 x 16 = 512
//                                                                     1    32     16
//                                                                 Tile_M Tile_N Tile_K

//BML的总load请求数量 = Tensor_N * (1个kernel) * Tensor_N * Taskctrl拆分的任务次数 =  64 * 1 * 2 * 512 = 65536 [B_LOAD]
//                       64          1             2         512

//CML的总load请求数量(Repeat row load) = (Tensor_N * 4 / 32) * 暂存区任务 = 64 * 4 / 32 * 32 = 256 [C_LOAD]
//                                        64       ^    ^     ^^^^ = Tile_M * Tile_N = 1 * 32 = 32
//                          存回去的数据格式为4byte---|    |----LLC的读数带宽是32byte/cycle

//CML的总store请求数量 = (APPLICATION_M * Tensor_N * 4 / 32) * 暂存区任务 = 49 * 64 * 4 / 32 * 32 = 12544 [D_STORE]
