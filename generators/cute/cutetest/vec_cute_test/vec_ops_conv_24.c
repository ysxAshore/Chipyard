...
void scale_after_operation_i32_to_i8_DimI_x_DimJ_64_ukernel_relu_shift_x(int32_t * input, int8_t * output, uint64_t stride_input, uint64_t stride_output,uint64_t shift_scale, uint64_t dim_I)
{
    ...
    __asm__ volatile (
        ...
        : "t0", "t1","t2","t3","t4","t5","t6","s2","s3","s4","s5","s6", "a3", "a4", "a5","a6","v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "memory" // 破坏描述符
    );
}
...
int main (int argc, char * argv[]) {
 ...
 //设置mstatus.VS = 1，其中mstatus[10:9]为mstatus.VS
    ...
    tiled_conv_CUTE_auto(conv_24_params, input, weights, bias, output,ACT_TYPE);
    uint64_t end = read_cycles();
    printf("conv_24 cycles: %lu \n", end - start);
