    ...
    with open("resnet50_conv_task.txt", "r") as f:
        ...
        for line in lines:
            ...
            with open("resnet50_cpu_int8_CUTE_chipyard_run_pure.c", "r") as f:
                ...
                # printf("conv_2 no cute cycles: %lu \n", end - start);
                # printf("conv_2 vec cycles: %lu \n",vec_time);
                content = content.replace("printf(\"conv_2 no cute cycles: %lu \\n\", end - start);", "printf(\"conv_" + num + " no cute cycles: %lu \\n\", end - start);")
                content = content.replace("printf(\"conv_2 vec cycles: %lu \\n\",vec_time);", "printf(\"conv_" + num + " vec cycles: %lu \\n\",vec_time);")
                content = content.replace("#include \"conv_2.h\"", "#include \"" + line + "\"")
                with open("vec_ops_only_" + num + ".c", "w") as f:
                    f.write(content)
